...
public class Take extends SWAffordance {
	 ...
	 * @author 	ram
	 * @author 	Asel (26/01/2017)
	 * @author 	Ariffin (27/05/2017) added code for grenade implementation
	...
	public void act(SWActor a) {
		if (target instanceof SWEntityInterface) {
			...
			Leave leave = new Leave(theItem, messageRenderer);
			target.addAffordance(leave);
			
			// if the item has a THROWABLE Capability, add the Throw affordance
			if (theItem.hasCapability(Capability.THROWABLE)) {
				Throw throwable = new Throw(theItem, messageRenderer);
				target.addAffordance(throwable);
			}
