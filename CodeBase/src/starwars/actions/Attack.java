	...
	public void act(SWActor a) {
		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item 
				...
				// if item carried is a lightsaber, but forcelevel is not high enough to wield it
				if ((itemCarried.getSymbol() == "†") && (a.getForceLevel() < LightSaber.getForceRequirement())) {
					if (targetIsActor) {
						targetActor.say("\t" + a.getShortDescription()
								+ " is not sufficiently trained to wield the "
								+ itemCarried.getShortDescription() 
								+ ". He punches " + targetActor.getShortDescription()
								+ " in the face instead.");
					}
				}
				else {
